...
public class QuadraticEquation {
    ...
    public Solution getSolution() {
        if (a == 0 && b == 0 && c == 0) {
            return new Solution(Status.ALL_REAL_NUMBERS, new double[]{});
        }

        double discr = Math.pow(b, 2) - 4 * a * c;
        if (discr < 0) {
            return new Solution(Status.NO_ROOTS_IN_REAL_NUMBERS, new double[]{});
        } else {
            if (discr == 0) {
                return new Solution(Status.HAS_ROOTS, new double[]{-b / (2 * a)});
            } else {
                return new Solution(Status.HAS_ROOTS, new double[]{
